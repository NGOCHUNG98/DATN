						...
						<div class="row">
							<div class="col-xs-12">
								...
								<form:form id="formSubmit" role="form" class="form-horizontal" modelAttribute="model">
									...
									<div class="form-group">
										<label class="col-sm-3 control-label no-padding-right" for="thumbnail"> Hình ảnh:  </label>

										<div class="col-sm-9">
											<%-- <form:textarea path="thumbnail" class="col-xs-10 col-sm-5" rows="5" placeholder="Nhập nội dung chi" id="thumbnail"/> --%>
											<form:input type="file" path="thumbnail"  class="col-xs-10 col-sm-5" rows="5" id="thumbnail"/>
										</div>
									</div>

									<%-- <div class="form-group">
											<label class="col-sm-3 control-label no-padding-right" for="thumbnail"> Ảnh: </label>
											<form:textarea path="thumbnail" id="thumbnail" class="col-xs-10 col-sm-5" rows="5"/>
									</div> --%>
			...
			<script>
				...
				$(document).ready(function(){
					editor = CKEDITOR.replace('thumbnail',{
						filebrowserUploadUrl: '/api',
					});
				}); */
				$(document).ready(function () {
					$('#btnAddOrEdit').click(function () {
					    var dataArray = {};
						var files = $("#formSubmit input[type='file']")[0].files[0];
						console.log(files);
					    if (files != undefined) {
				...
				function editNew(data){
					$.ajax({
						...
						dataType: 'json',
						success: function (result) {
            				window.location.href = "${motoURL}?page=1&limit=5&message=update_success";
           			 	},
						error: function (error) {
							window.location.href = "${motoURL}?page=1&limit=5&message=error_system";
						}
