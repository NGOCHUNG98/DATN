			...
			<div class="table-responsive">
				<table class="table table-bordered">
					...
					<tbody>
						...
						<c:set var="s" value="${s+ pr.motocrycleDTO.price * pr.motocrycleDTO.quantity}"></c:set>
							<tr>
								<td>
									<a href='<c:url value="/gio-hang/xoa-hang?id=${pr.motocrycleDTO.id}"/>'>Remove</a>
								</td>
								<td>${pr.motocrycleDTO.name}</td>
								<td>${pr.motocrycleDTO.description}</td>
								<td><img class="card-img-top thumnail-image"
									src='<c:url value="/resources/images/${pr.motocrycleDTO.thumbnail}"/>' /></td>
								<td><fmt:formatNumber type="number" maxFractionDigits="3"
								value="${pr.motocrycleDTO.price}" />
								<small class="text-muted">đ</small></td>
								<td>${pr.motocrycleDTO.quantity}</td>
							</tr>
						</c:forEach>
						<tr>
				...
				<a href='<c:url value="/thong-tin-khach-hang"/>'>Thanh toán</a>
			</div>
		</div>
	</div> --%>
	<div class="cart_area section_padding_100 clearfix">
            <div class="container">
                <div class="row">
                    ...
                    <div class="col-12">
                    <form action='<c:url value="/update-quantity"/>' method="post">
